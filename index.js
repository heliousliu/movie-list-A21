(function () {
  ...
  const DEFAULT_START_PAGE = 1 //目前頁數初始值:1
  const DEFAULT_DISPLAY_MODE = 'card'  //預設顯示方式:卡片
  const data = []
  let paginationData = []
  ...
  const displayCardModeBtn = document.getElementById('card-mode')
  const displayListModeBtn = document.getElementById('list-mode')
  ...
  displayCardModeBtn.addEventListener('click', event => {
    displayMode = 'card'
    getPageData(currentPage)
  })

  displayListModeBtn.addEventListener('click', event => {
    displayMode = 'list'
    getPageData(currentPage)
  })
  ...
  function displayDataList(data) {
    ...
    let htmlContent = ''
    if (displayMode === 'list') {
      htmlContent = `<ul class="list-group list-group-flush" style="width:100%;">`
